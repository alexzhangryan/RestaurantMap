...
.count{font-size:12px;color:var(--sub);padding:5px 9px 5px 3px;margin-top:6px;pointer-events:auto;
  background:none;border:0;font-family:inherit;display:inline-flex;align-items:center;gap:3px;
  cursor:pointer;border-radius:999px;-webkit-tap-highlight-color:transparent}
.count:active{background:color-mix(in srgb,var(--sub) 14%,transparent);transform:scale(.97)}
.count .chev{opacity:.55;font-size:11px;transform:translateY(-.5px)}
...
/* bottom sheets (filters + list) — always mounted, driven by transform/opacity so
   opening/closing can glide instead of hard-cutting via display:none. */
#fsheet,#lsheet{position:fixed;inset:0;z-index:3000;pointer-events:none}
#fsheet.open,#lsheet.open{pointer-events:auto}
#fsheet .backdrop,#lsheet .backdrop{position:absolute;inset:0;background-color:rgba(0,0,0,0)}
#fsheet.open .backdrop,#lsheet.open .backdrop{background-color:rgba(0,0,0,.4)}
#fsheet .sheet,#lsheet .sheet{position:absolute;left:0;right:0;bottom:0;max-height:82vh;display:flex;flex-direction:column;
  background:var(--panel);border-radius:20px 20px 0 0;box-shadow:0 -8px 30px rgba(0,0,0,.3);transform:translateY(100%)}
#fsheet.open .sheet,#lsheet.open .sheet{transform:translateY(0)}
@media (prefers-reduced-motion:no-preference){
  #fsheet .backdrop,#lsheet .backdrop{transition:background-color .25s ease}
  #fsheet .sheet,#lsheet .sheet{transition:transform .32s cubic-bezier(.22,.61,.36,1)}
}
#fsheet .fhead,#lsheet .lhead{display:flex;align-items:center;justify-content:space-between;padding:14px 8px 12px 18px;
  border-bottom:1px solid var(--line)}
#fsheet .fhead h2,#lsheet .lhead h2{margin:0;font-size:16px}
#fsheet .fclear{background:none;border:0;color:var(--accent);font-size:14px;font-weight:600;cursor:pointer;padding:6px 4px}
#fsheet .fclose,#lsheet .lclose{background:none;border:0;color:var(--sub);font-size:20px;cursor:pointer;padding:4px 10px}
...
/* list / browse sheet */
#lsheet .lbody{overflow-y:auto;-webkit-overflow-scrolling:touch;padding:4px 10px calc(8px + env(safe-area-inset-bottom))}
.progress-wrap{padding:12px 18px 14px;border-bottom:1px solid var(--line)}
.progress-stat{display:flex;justify-content:space-between;font-size:12.5px;color:var(--sub);margin-bottom:7px;font-weight:600}
.progress-stat b{color:var(--ink)}
.progress-bar{height:7px;border-radius:999px;background:var(--line);overflow:hidden}
.progress-fill{height:100%;border-radius:999px;width:0%;background:linear-gradient(90deg,var(--visited),#6bd98a)}
@media (prefers-reduced-motion:no-preference){.progress-fill{transition:width .5s cubic-bezier(.22,.61,.36,1)}}
.lrow{display:flex;align-items:center;gap:12px;padding:9px 8px;border-radius:14px;cursor:pointer;
  -webkit-tap-highlight-color:transparent}
.lrow:active{background:var(--bg);transform:scale(.99)}
.lrow .lthumb{position:relative;flex:0 0 50px;width:50px;height:50px;border-radius:12px;overflow:hidden;
  background:linear-gradient(90deg,var(--line) 25%,color-mix(in srgb,var(--line) 55%,var(--panel)) 50%,var(--line) 75%);
  background-size:250% 100%;animation:shimmer 1.4s ease-in-out infinite;
  display:flex;align-items:center;justify-content:center;font-size:19px}
.lrow .lthumb.loaded{animation:none;background:var(--line)}
.lrow .lthumb img{position:absolute;inset:0;width:100%;height:100%;object-fit:cover;opacity:0}
@media (prefers-reduced-motion:no-preference){.lrow .lthumb img{transition:opacity .25s ease}}
.lrow .lthumb img.loaded{opacity:1}
.lrow .lmain{flex:1;min-width:0}
.lrow .lname{font-size:14px;font-weight:600;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;
  display:flex;align-items:center;gap:5px}
.lrow .lname .lvisited{color:var(--visited);font-size:12px;flex:0 0 auto}
.lrow .lmeta{font-size:12px;color:var(--sub);white-space:nowrap;overflow:hidden;text-overflow:ellipsis;margin-top:1px}
.lrow .ldist{flex:0 0 auto;font-size:12px;font-weight:700;color:var(--sub);text-align:right}
.lempty{padding:48px 24px;text-align:center;color:var(--sub);font-size:14px;line-height:1.5}
...
.leaflet-marker-icon:active .pin{transform:scale(.82)}
...
.me-pulse{position:absolute;left:50%;top:50%;width:16px;height:16px;border-radius:50%;
  background:rgba(10,132,255,.35);transform:translate(-50%,-50%);animation:me-pulse 2.2s ease-out infinite}
...
@keyframes me-pulse{0%{transform:translate(-50%,-50%) scale(1);opacity:.8}100%{transform:translate(-50%,-50%) scale(3.4);opacity:0}}
...
.card .photowrap{position:relative;margin:0 0 10px;cursor:zoom-in;border-radius:12px;overflow:hidden;
  background:linear-gradient(90deg,var(--line) 25%,color-mix(in srgb,var(--line) 55%,var(--panel)) 50%,var(--line) 75%);
  background-size:250% 100%;animation:shimmer 1.4s ease-in-out infinite}
.card .photowrap.loaded{animation:none;background:var(--line)}
.card .photo{width:100%;height:132px;object-fit:cover;border-radius:12px;display:block;opacity:0}
.card .photo.loaded{opacity:1}
@media (prefers-reduced-motion:no-preference){.card .photo{transition:opacity .25s ease}}
...
.visited-btn{position:relative;display:flex;align-items:center;justify-content:center;gap:8px;width:100%;margin:2px 0 9px;
...
.visited-btn.bump{animation:visited-bump .32s ease}
@keyframes visited-bump{30%{transform:scale(1.04)}100%{transform:scale(1)}}
.confetti{position:absolute;top:50%;left:50%;width:6px;height:6px;border-radius:50%;pointer-events:none;
  animation:confetti-burst .6s ease-out forwards}
@keyframes confetti-burst{to{transform:translate(var(--tx),var(--ty)) scale(0);opacity:0}}

/* milestone toast */
#toast{position:fixed;left:50%;bottom:calc(env(safe-area-inset-bottom) + 92px);z-index:5000;
  background:var(--ink);color:var(--bg);padding:12px 20px;border-radius:999px;font-size:13.5px;font-weight:700;
  box-shadow:0 10px 28px rgba(0,0,0,.35);opacity:0;pointer-events:none;white-space:nowrap;
  transform:translate(-50%,14px)}
#toast.show{opacity:1;transform:translate(-50%,0)}
@media (prefers-reduced-motion:no-preference){#toast{transition:opacity .3s ease, transform .3s cubic-bezier(.22,.61,.36,1)}}
...
<div class="topbar">
  ...
  <button class="count" id="count" type="button"><span id="countText"></span><span class="chev">›</span></button>
</div>
<div id="fsheet" aria-hidden="true">
  ...
  <div class="sheet">
    ...
    <div class="fbody">
      ...
      <div class="fsec">
        <h3>Progress</h3>
        <div class="fchips" id="fProgress">
          <div class="fchip" data-unvisited="1" aria-pressed="false">🎯 Not visited yet</div>
        </div>
      </div>
      <div class="fsec">
        <h3>Hours</h3>
...
<div id="lsheet" aria-hidden="true">
  <div class="backdrop" id="lsheetBackdrop"></div>
  <div class="sheet">
    <div class="lhead">
      <h2>Places</h2>
      <button class="lclose" id="lclose" aria-label="Close">✕</button>
    </div>
...
// --- distance (needs meMarker from the locate/live-dot section below) ---
...
function fmtDist(mi){ return mi < 0.1 ? Math.round(mi*5280) + " ft" : mi.toFixed(1) + " mi"; }
function distTo(p){ return meMarker ? haversineMi(meMarker.getLatLng(), p) : null; }

// --- gamification: visited progress + list-sheet rows ---
function visitedStats(){
  const total = PLACES.length;
  const done = PLACES.filter(p => VISITED[pkey(p)]).length;
  return {total, done, pct: total ? Math.round(done/total*100) : 0};
}
function listRowHtml(p, dist){
  const thumb = p.photo
    ? `<div class="lthumb"><img src="${p.photo}" alt="" loading="lazy" onload="this.classList.add('loaded');this.parentElement.classList.add('loaded')"></div>`
    : `<div class="lthumb loaded">${p.emoji}</div>`;
  const meta = [p.cat, p.price, p.rating ? "★ " + p.rating : ""].filter(Boolean).join(" · ");
  const distTxt = dist != null ? `<div class="ldist">${fmtDist(dist)}</div>` : "";
  const vis = VISITED[pkey(p)] ? `<span class="lvisited">✓</span>` : "";
  return `<div class="lrow" data-k="${esc(pkey(p))}">
    ${thumb}
    <div class="lmain"><div class="lname">${esc(p.name)}${vis}</div><div class="lmeta">${esc(meta)}</div></div>
    ${distTxt}
  </div>`;
}
function renderList(){
  const items = VIS.map(m => m.__p);
  const here = meMarker ? meMarker.getLatLng() : null;
  const lbody = document.getElementById("lbody");
  if (!items.length){
    lbody.innerHTML = `<div class="lempty">No places match your filters.</div>`;
  } else if (here){
    lbody.innerHTML = items
      .map(p => ({p, d: haversineMi(here, p)}))
      .sort((a, b) => a.d - b.d)
      .map(x => listRowHtml(x.p, x.d))
      .join("");
  } else {
    lbody.innerHTML = items.map(p => listRowHtml(p, null)).join("");
  }
  const st = visitedStats();
  document.getElementById("progressFill").style.width = st.pct + "%";
  document.getElementById("progressStat").innerHTML = `<b>${st.done} / ${st.total}</b> visited &nbsp;·&nbsp; ${st.pct}%`;
}
document.getElementById("lbody").addEventListener("click", e => {
  const row = e.target.closest(".lrow"); if(!row) return;
  const m = byKey[row.dataset.k]; if(!m) return;
  closeModal(lsheet);
...
function popupHtml(p){
  ...
  const photo = p.photo
    ? `<div class="photowrap" data-k="${esc(pkey(p))}"><img class="photo" src="${p.photo}" alt="" loading="lazy"
         onload="this.classList.add('loaded');this.parentElement.classList.add('loaded')">${badge}</div>`
    : "";
  ...
  return `<div class="card">
    ...
    <h3>${esc(p.name)}</h3>
    <div class="meta">${esc(p.cat)}${price}${stars}${dist}</div>
    ${feat}${known}
...
const markers = PLACES.map(p => {
  ...
  m.__p = p;
  // content is a function (not a precomputed string) so today's-hours + distance-from-me
  // stay correct no matter when in the session the popup actually gets opened
...
function render(){
  ...
  for (const m of markers){
    ...
    if (ONLY_UNVISITED && VISITED[pkey(p)]) continue;
  ...
  document.getElementById("countText").textContent = shown + " places";
  updateFilterBadge();
  scheduleDeclutter();
  if (lsheet.classList.contains("open")) renderList();
}
...
function openSheet(){ openModal(fsheet); }
function closeSheet(){ closeModal(fsheet); }
...
document.getElementById("count").addEventListener("click", () => { renderList(); openModal(lsheet); });
document.getElementById("lclose").addEventListener("click", () => closeModal(lsheet));
document.getElementById("lsheetBackdrop").addEventListener("click", () => closeModal(lsheet));
...
function syncSheetUI(){
  ...
  document.querySelector("#fProgress .fchip").setAttribute("aria-pressed", ONLY_UNVISITED);
...
document.getElementById("fclear").addEventListener("click", () => {
  ...
  HOURS_MODE = "any"; PICK_DAY = "today"; PICK_TIME = "18:00"; ONLY_UNVISITED = false;
  syncSheetUI();
  render();
});
document.getElementById("fProgress").addEventListener("click", e => {
  const c = e.target.closest(".fchip"); if(!c) return;
  ONLY_UNVISITED = !ONLY_UNVISITED;
  c.setAttribute("aria-pressed", ONLY_UNVISITED);
  render();
});
...
// milestone toast — a light celebratory nudge the first time you cross 25/50/75/100% visited
...
const MKEY = "milestone_v1";
let milestoneSeen = 0;
try { milestoneSeen = parseInt(localStorage.getItem(MKEY) || "0", 10) || 0; } catch(e){}
function checkMilestone(){
  const st = visitedStats();
  const hit = [25, 50, 75, 100].filter(m => st.pct >= m && milestoneSeen < m).pop();
  if (!hit) return;
  milestoneSeen = hit;
  try { localStorage.setItem(MKEY, String(hit)); } catch(e){}
  const msg = hit === 100 ? `🏆 All ${st.total} places visited!` : `🎉 ${hit}% visited — ${st.done}/${st.total} places!`;
  showToast(msg);
}

// small confetti burst from the mark-visited button, for a bit of delight
function burstConfetti(btn){
  const colors = ["#2fb344", "#0a84ff", "#e8590c", "#ffd43b"];
  for (let i = 0; i < 10; i++){
...
document.addEventListener("click", e => {
  ...
  if (now){ burstConfetti(b); checkMilestone(); }
  if (lsheet.classList.contains("open")) renderList();
});
...
// Tapping locate starts a continuous watch (not a one-shot fix) so the dot tracks you
// as you move, like Apple/Google Maps — but only the FIRST fix recenters the map, so
// later updates don't fight you if you've panned away to look at something else.
let recenterOnFix = true;
document.getElementById("locate").addEventListener("click", () => {
  ...
  recenterOnFix = true;
  map.locate({watch:true, setView:false, maxZoom:15, enableHighAccuracy:true});
});
map.on("locationfound", e => {
  if(meMarker){ meMarker.setLatLng(e.latlng); }
    ...
    meMarker = L.marker(e.latlng, {interactive:false, zIndexOffset:10000, icon: L.divIcon({
      ...
      html:`<div class="me"><div class="me-pulse"></div>${CONE}<div class="me-dot"></div></div>`})}).addTo(map);
  }
  updateCone();
  if(recenterOnFix){ recenterOnFix = false; map.setView(e.latlng, Math.max(map.getZoom(), 15), {animate:true}); }
  if(lsheet.classList.contains("open")) renderList();
});
